...
1. `F44-CONT-05`
   Scope: launch approved state pages from the manifest with the same standardized section set, disclosure rules, citation support, and state-specific evidence ownership required by the scalable content system.
   Conflict notes: shared manifest/template surfaces in `content/manifest.*`, new `content/states/*`, page templates, and sitemap/llms outputs; sequence carefully with any concurrent crawl, content, or freshness changes that touch the same inventories.
2. `F44-EXP-01`
...
Other `Status: ready` tasks are also unblocked on current `main`, but `F44-QA-02` remains downstream of `F44-CNV-02`, and `F44-CONT-07` should stay sequenced behind `F44-CONT-05` plus the current quality-control gates now that industry launch and freshness reporting are both complete.
