...
| 2026-07-27 | customer-facing cleanup | Removed internal roadmap/ticket vocabulary from customer-facing rendered copy: link-module group titles (`Hub page` -> `Main financing guide`, `Related routes` -> `Related financing paths`, `Next step` -> `Where to go next`; default section heading `Explore related routes` -> `Explore related financing paths`), legal/contact placeholder and indexing-note copy in `src/lib/legal.js` + `src/pages/legal.js` (rendered `F44-GOV-02` / `F44-SEC-01`, `governance draft`, and `Controlled TBD` wording replaced with preview/being-verified language), link-graph privacy/contact fallback descriptions, `canonical program page` -> `program guide` in `content/pages/financing.json`, and `related routes` -> `related financing paths` in `content/pages/business-acquisition.json`. Placeholder semantics preserved: values stay visibly not-final and intentionally withheld until verified; no identity, contact, or outcome facts added; `docs/link-graph.md` regenerated. | Acceptance grep over `dist` HTML for F44-* ids / governance draft / controlled TBD / Hub page / Related routes / canonical program page returns no matches (also clean case-insensitively); `npm test` 79/79; `npm run qa:release` 17/17 pass (a11y 11 passed + 4 skipped, release smoke 6 passed). | `agent/f44-copy-cleanup` |
