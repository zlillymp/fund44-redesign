...
1. `F44-QA-01`
   Scope: rebuild release-readiness automation and CI around the shipped validator, prerender, route-smoke, analytics, accessibility, and mobile checks.
   Conflict notes: shared test/config/workflow surfaces in `package.json`, `playwright.config.mjs`, `tests/**`, and future CI definitions; sequence carefully with any concurrent browser-test expansion.
2. `F44-CONT-02`
   Scope: launch approved national financing hubs from the manifest on top of the shipped scalable template contract, citation rules, link-graph requirements, and analytics tags.
   Conflict notes: shared manifest/content/template surfaces in `content/manifest.*`, new `content/financing/*`, page templates, and internal-link/disclosure rendering; sequence carefully with other content-launch tasks so manifest rows and template changes do not collide.
3. `F44-CONT-03`
   Scope: launch approved use-case pages from the manifest using the same standardized section set, evidence fields, internal links, and measurement metadata now required by `F44-CONT-01`.
   Conflict notes: shared scalable-content surfaces in `content/manifest.*`, new `content/use-cases/*`, `content/templates/*`, and page templates; avoid racing `F44-CONT-02` on common template and manifest files.

Other `Status: ready` tasks are also unblocked on current `main`, but they rank behind the top three above because `F44-CONT-04` and `F44-CONT-05` follow the same launch pattern as `F44-CONT-02` and `F44-CONT-03`, `F44-CONT-06` is more useful after additional canonical pages exist, and `F44-EXP-01` / `F44-CNV-02` / `F44-QA-02` build on the release and expansion layers those tasks enable.
