...
1. `F44-CONT-04`
   Scope: launch approved industry pages from the manifest using the shipped scalable template contract, citation rules, link-graph requirements, and measurement metadata already proven on national financing and use-case clusters.
   Conflict notes: highest-collision surfaces are `content/manifest.*`, new `content/industries/*`, shared page templates, and internal-link rendering; do not overlap with any concurrent manifest or template refactors.
2. `F44-CONT-05`
   ...
   Conflict notes: shared manifest/template surfaces in `content/manifest.*`, new `content/states/*`, page templates, and sitemap/llms outputs; sequence after or separately from `F44-CONT-04` if both would touch common template or manifest rows.
3. `F44-CONT-06`
   Scope: add freshness review metadata and stale-content reporting across the now-expanded canonical content inventory so future industry, state, and later metro launches inherit review ownership and release-visible stale signals.
   Conflict notes: shared governance surfaces in `content/manifest.*`, `content/citations.*`, and new freshness-report tooling/docs; avoid racing any active content-launch task that is still reshaping manifest fields or citation structures.

Other `Status: ready` tasks are also unblocked on current `main`, but `F44-EXP-01` and `F44-CNV-02` now rank behind the top three above because the scalable content system has shipped through national financing and use-case clusters, the next missing launch surfaces are industry and state manifests, and the freshness workflow becomes more valuable before any further programmatic expansion. `F44-QA-02` remains downstream of `F44-CNV-02`.
