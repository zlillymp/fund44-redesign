...
1. `F44-EXP-01`
   Scope: add the experimentation harness, exposure tracking, kill switches, and guardrail documentation on top of the now-stable route/content/analytics/freshness foundation without bypassing governed claims, disclosure, or consent boundaries.
...
2. `F44-CNV-02`
   Scope: build contextual product funnels that preserve page intent and measurement context now that the route/content/citation/freshness systems are stable across financing, use-case, and industry entry pages.
...
3. `F44-CONT-07`
   Scope: expand into metro and other approved programmatic combinations only after the financing, use-case, industry, and first state-page launches have proven the structured-content, citation, freshness, crawl, and internal-link quality gates on current `main`.
   Conflict notes: highest-collision surfaces are approved manifests, `content/*` inventories, `content/manifest.*`, `content/citations.*`, `docs/link-graph.md`, generated crawl assets, and the freshness/release-gate scripts that now govern indexability for scaled content.

Other `Status: ready` tasks are also unblocked on current `main`, but `F44-QA-02` remains downstream of `F44-CNV-02`, and `F44-EXP-02` should stay sequenced behind `F44-EXP-01`, `F44-CNV-02`, and `F44-QA-02`.
