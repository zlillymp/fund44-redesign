...
3. `F44-CONT-01`
   Scope: standardize the reusable section set and validation gates for financing, use-case, industry, and state templates on top of the structured content and citation/link-graph foundations already shipped.
   Conflict notes: shared schema/template/validator surfaces in `content/schema/*`, `content/templates/*`, page templates, and `scripts/validate-content.mjs`; complete this before `F44-CONT-02` through `F44-CONT-05`.
