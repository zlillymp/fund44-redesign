...
- [x] `F44-EXP-01` `P1` Build an experimentation harness with guardrails and exposure tracking.
  Status: done - claude/agent/f44-exp-01 (2026-07-27)
...
| 2026-07-27 | `F44-EXP-01` | Built the experimentation harness on the existing `F44-MEA-02` taxonomy (no event or property changes): new `src/lib/experiments.js` with a frozen empty registry, definition validation (snake_case ids, surface enum nav/cta/funnel/trust/mobile, explicit enabled flag, weighted variants, required guardrail metrics), deterministic FNV-1a session bucketing, and layered kill switches (registry flag, per-experiment `__FUND44_EXPERIMENT_KILLSWITCH__`, global `__FUND44_EXPERIMENTS_DISABLED__`); `src/lib/analytics.js` now injects active assignments into `experiment_ids` inside `sharedFields()` so the shared property is genuinely global, and exports `trackExperimentExposure` which refuses to emit for killed or inactive experiments; new `docs/experiment-rules.md` binds experiments to consent/disclosure immutability, claims/design governance, existing-KPI guardrails, and a launch checklist. Registry ships empty, so runtime behavior is unchanged until an experiment entry is reviewed in. | `npm test` 87/87 including new `tests/experiments.test.mjs` (8 tests: empty-registry default, definition validation, deterministic assignment + spread, zero-weight exclusion, kill switches, disabled entries, `experiment_ids` on all events, exposure emit/refuse — sample exposure payload asserted with `experiment_id`/`variant_id`/`surface` and `experiment_ids=['nav_cta_label_test']`); `npm run validate:analytics` pass (24 events / 31 routes); `npm run qa:release` pass 17/17. | `branch:agent/f44-exp-01` |
