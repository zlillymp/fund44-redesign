...
test('live mode follows the enabled live sequence and stops at contact capture without provided contact details', () => {
  const { state, visited } = walkToTerminalStep({ requestedMode: 'live' }, ELIGIBILITY_MODES.live);

  assert.equal(state.currentStepId, STEP_IDS.contactCapture, 'live mode must stop at contact capture when contact details are not provided');
  assert.equal(state.outcome, null, 'live mode must not produce an outcome before contact details are submitted');
  assert.ok(visited.includes(STEP_IDS.contactCapture), 'enabled live mode must reach contact capture');
  assert.ok(!visited.includes(STEP_IDS.liveUnavailable), 'enabled live mode must not visit the unavailable step');
  assert.ok(!visited.includes(STEP_IDS.outcome), 'live mode must not reach the outcome step before contact details are provided');
});
